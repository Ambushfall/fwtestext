...
function mainBuildContent (ctx) {
  ...
  const addButtonToDiv = itemDiv => {
    ...
    if (copyButton && !itemDiv.querySelector('.trade-button')) {
      ...
      if(
        backgroundSize !== 'calc(1 * var(--cellSize)) calc(1 * var(--cellSize))' && 
  ...
  const simulateCopyButtonClick = copyButton => {
    ...
    navigator.clipboard.readText().then(async text => {
      ...
      let itemData = processCopiedData(text)

      // Wait for the trade stats to be processed and added to itemData
      await new Promise(resolve => {
        browser.storage.local.get('tradeStats', data => {
          if (data.tradeStats) {
            processItemDataWithTradeStats(itemData, data.tradeStats)
            resolve('') // Continue after processing is done
          } else {
            console.error('No tradeStats found in storage.')
            resolve('') // Continue even if tradeStats aren't found
          }
        })
      })
      ...
      browser.runtime.sendMessage(
        { action: 'checkAndSendPostRequest', requestBody: requestBody },
        response => {
  ...
  // This function constructs the JSON body for the POST request
  function constructJsonFromParsedData (parsedData) {
    const { baseType, sockets, poeInternalModifierValues, rarity } = parsedData
  ...
  const processMod = (line, defaultKey = '') => {
  ...
  // Helper function to check if this is a non-unique cluster jewel
  const isNonUniqueClusterJewel = (rarity, baseType) => {
    const isClusterJewel =
      baseType &&
      (baseType.includes('Large Cluster Jewel') ||
        baseType.includes('Medium Cluster Jewel') ||
        baseType.includes('Small Cluster Jewel'))
    return (rarity === 'Rare' || rarity === 'Magic') && isClusterJewel
  }

  // Helper function to process sockets
  const processSockets = socketsData => {
  ...
  const processInfoGroup = (lines, itemData) => {
  ...
  const normalizeValueForTradeStats = value => {
  ...
  const tryMatchWithOptions = (baseText, optionsKey, modifierText) => {
    ...
    let optionMatch = optionsArray.find(option => option.text === modifierText)
  ...
  const handleSpecialCases = (tradeStats, key, value, baseType) => {
  ...
  const processItemDataWithTradeStats = (itemData, tradeStats) => {
    const processedData = {}
