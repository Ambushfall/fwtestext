...
export default defineBackground({
  ...
  main() {
    ...
    async function messageHandler(
      ...
      switch (message.action) {
        case "parseCopiedText":
          ...
          let { tradeStats } = await browser.storage.local.get("tradeStats");
          if (tradeStats) {
            processItemDataWithTradeStats(itemData, tradeStats);
            // Construct the JSON body for the POST request
            const requestBody = constructJsonFromParsedData(itemData);
            await checkAndSendPostRequest(requestBody, sender, sendResponse);
          } else {
    ...
    async function fetchAndStoreStatsData() {
      try {
        let response = await fetch(
          "https://www.pathofexile.com/api/trade/data/stats",
        );
        let data = await response.json();
        await browser.storage.local.set({ tradeStats: data });
      } catch (error) {
    ...
    async function checkAndSendPostRequest(
      requestBody: any,
      sender: Browser.runtime.MessageSender,
      sendResponse: (response?: any) => void,
    ) {
      let { league } = await browser.storage.local.get("league");
      ...
      if (cookie) {
        ...
        try {
          ...
          const newUrl = `https://www.pathofexile.com/trade/search/${league}/${id}`;

          // Send the new URL back to the content script
          return sendResponse({ status: "success", data: newUrl });
        } catch (error) {
          console.warn("POST request failed:", error);
          return sendResponse({ status: "error", error: error.message });
        }
      } else {
        console.warn("POESESSID cookie not found.");
        return sendResponse({
        ...
        if (lines.length > 0) {
          ...
          switch (true) {
            ...
            case firstLine.startsWith("Sockets:"):
              ...
              itemData.sockets = processSockets(
                firstLine.split("Sockets: ")[1],
              );
              break;
    ...
    function queryTradeStats(
      ...
      const tryMatch = (valueToMatch) => {
        ...
        match = matchedArray.entries.find(
          (entry) => entry.text === modifiedValue,
        );
        if (match) {
